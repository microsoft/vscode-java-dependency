...
import { CopilotCancellationError, GetImportClassContentError, GetProjectDependenciesError } from "./utils";
...
export namespace CopilotHelper {
    ...
    export async function resolveLocalImports(fileUri: Uri, cancellationToken?: CancellationToken): Promise<INodeImportClass[]> {
        ...
        try {
            ...
            if (error.message === 'Operation cancelled') {
                sendError(new CopilotCancellationError());
                return [];
            }
            sendError(new GetImportClassContentError('Failed to get import class content: ' + ((error as Error).message || "unknown_error")));
            return [];
        }
    }
    ...
    export async function resolveProjectDependencies(projectUri: Uri, cancellationToken?: CancellationToken): Promise<IProjectDependency> {
        ...
        try {
            ...
            sendError(new GetProjectDependenciesError('Failed to get project dependencies: ' + ((error as Error).message || "unknown_error")));
            return {};
        }
    }
}
